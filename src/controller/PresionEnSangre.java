...
public class PresionEnSangre implements Initializable{
    ...
    void enseñarDatos(ActionEvent event) {
		...
		if(userLogged.rol_tipo.equals("Conductor") || userLogged.rol_tipo.equals("conductor") || userLogged.rol_tipo.equals("Conductora") || userLogged.rol_tipo.equals("conductora")) {
	    	...
	    	if(inputFecha.getText().isEmpty()) {
	    		...
	    		try {
	    			...
	    			while (rs.next()) {
		        		...
		        		if(inputFecha.getText().equals(fechaString) && nombreUser.getText().equals(nombre)) {
		        			errorFecha.setText("");
	    	...
	    	if(event.getSource() == botonFecha) {
	            outputDatos.setText("Datos de " +datosVector.get(posicion).nombre+ ": " +datosVector.get(posicion).nivelhb+ "\n" );
	    	}
